...
namespace TakeAPeek_server.Controllers
{
    ...
    public static class UserEndpoints
    {
        ...
        {
            app.MapGet("/users", async (IUserService userService) => await userService.GetAllUsers()).RequireAuthorization();

            app.MapGet("/users/{id}", async (int id, IUserService userService) => await userService.GetUser(id)).RequireAuthorization();
            ...
            app.MapDelete("/users/{id}", async (int id, IUserService userService) => await userService.DeleteUser(id)).RequireAuthorization("Editor", "Admin");
        }
    }
